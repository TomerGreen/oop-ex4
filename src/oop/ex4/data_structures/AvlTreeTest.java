...
public class AvlTreeTest {
    ...
    public void testSimpleOperations() {
        ...
        assertTrue("Contains failed to return 1 for leaf depth", tree.contains(-1) == 1);
        assertTrue("Contains failed to return 0 for leaf depth", tree.contains(0) == 0);
        assertTrue("Failed to add non-existent value", tree.add(1));
        assertTrue("Contains failed to return 1 for leaf depth", tree.contains(1) == 1);
        assertTrue("Contains failed to return 0 for root height", tree.contains(0) == 0);
        assertTrue("Failed to add non-existent value", tree.add(100));
        assertTrue("Contains found non-existent value", tree.contains(2) == -1);
        assertTrue("Contains did not return 2 for existent value depth", tree.contains(100) == 2);
        assertTrue("Contains did not return 1 for existent value depth", tree.contains(1) == 1);
        assertTrue("Contains did not return 1 for existent value depth", tree.contains(-1) == 1);
        assertTrue("Contains did not return 2 for existent value depth", tree.contains(0) == 0);
        assertTrue("Failed to add value: 200", tree.add(200));
        assertTrue("Failed to return 0 for root depth", tree.contains(0) == 0);
        assertTrue("Failed to return 1 for -1 depth", tree.contains(-1) == 1);
        assertTrue("Failed to return 1 for 100 depth", tree.contains(100) == 1);
        assertTrue("Failed to return 2 for 1 depth", tree.contains(1) == 2);
        assertTrue("Failed to return 2 for 200 depth", tree.contains(200) == 2);
